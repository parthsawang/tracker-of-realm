...
export function Layout({ children }: LayoutProps) {
  ...
  // Check if current page should have full width (hide sidebar)
  const isFullWidth = ["/system", "/apps", "/tracker"].includes(
    location.pathname,
  );
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      {/* Main content */}
      <div className={isFullWidth ? "w-full -mt-0.5" : "lg:pl-64 -mt-0.5"}>
        {/* Top navigation */}
                ...
                <p className="text-sm text-muted-foreground">
                  {isFullWidth
                    ? location.pathname === "/system"
                      ...
                      : location.pathname === "/apps"
                        ? "Track application usage, set time limits, and manage focus modes"
                        : location.pathname === "/tracker"
        ...
        {/* Page content */}
        <main className={isFullWidth ? "p-4" : "p-6"}>{children}</main>
      </div>
    </div>
  );
}
